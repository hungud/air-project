namespace App.Base
{
    namespace BaseProxyHandler
    {
        ...
        /// Summary     :   BaseProxyHandler
        ///*************************************************
        /// </summary>
        public class BaseProxyHandler : IHttpHandler, IDisposable
        {
            ...
            public void ProcessRequest(HttpContext context)
            {
                ...
                {
                    context.Response.Write("Hello and Welcome Proxy Applications");
                    return;
                }
            ...
            private string getTokenFromConfigFile(string uri)
            {
                try
                {
                    ...
                    {
                        throw new ApplicationException("WebSecuredService.config file does not exist at application root, or is not readable.");
                    }
                }
                                    ...
                                    while (webResponse_401 == null || webResponse_401.StatusCode != HttpStatusCode.OK)
                                    {
                                        ...
                                        catch (System.Net.WebException webex)
                                        {
                                            ...
                                            if (webexResponse.StatusCode == HttpStatusCode.Unauthorized)
                                            {
                                                ...
                                                {
                                                    webRequest_401 = (HttpWebRequest)HttpWebRequest.Create(url_401);
                                                    webRequest_401.Credentials = new System.Net.NetworkCredential(si.Username, si.Password, si.Domain);
                                                }
                    ...
                    if (config != null)
                    {
                        foreach (ServerItem si in config.ServerItems)
                            if (url.StartsWith(si.Url, true, null))
                            {
                                ...
                                if (theToken == null)
                                {
                                    string tokenServiceUrl = string.Format("{0}?request=getToken&username={1}&password={2}", si.TokenUrl, si.Username, si.Password);
                                    int timeout = 600;
                                    if (si.Timeout > 0)
                                        timeout = si.Timeout;
            ...
            #endregion BaseHttpHandler Block
        ...
        public class ProxyConfig
        {
            ...
            {
                ProxyConfig config = HttpRuntime.Cache["WebSecuredService"] as ProxyConfig;
                if (config == null)
                {
                    ...
                    if (config != null)
                    {
                        ...
                        HttpRuntime.Cache.Insert("WebSecuredService", config, dep);
                    }
                }
                return config;
            }
            ...
            {
                return context.Server.MapPath("~/WebSecuredService.config");
            }
            #endregion
