...
function GetUserContactDetails() {
    /* Check Company Type */
    //
    var servicepath = CommonConfiguration.searchBoxService + "/GetContactDetailsByUserID";


    var UserID = localStorage.getItem("loginUID");
    $.ajax({
        cache: false,
        url: servicepath,
...
function GetOriginDestination() {
    try {
        function ResultCallBackSuccess(e, xhr, opts) {
            ...
            if (sessionStorage["departarraynew"] !== "" && sessionStorage["departarraynew"] !== null) {
                sessionStorage.setItem("departarray", sessionStorage["departarraynew"]);
            ...
            if (sessionStorage["arrivalarraynew"] !== "" && sessionStorage["arrivalarraynew"] !== null) {
                sessionStorage.setItem("arrivalarray", sessionStorage["arrivalarraynew"]);
        ...
        $.each(myPassengerDetailsListArray, function (dt_key, dt_value) {
            if (parseInt(dt_value.Adults) > 0) {
                for (var i = 0; i < parseInt(dt_value.Adults) ; i++) {
                    ...
                    //var PassangerName = $("#txtFirstNameAdults" + i + "").val().trim() + $("#txtMiddleNameAdults" + i + "").val().trim() + $("#txtLastNameAdults" + i + "").val().trim();
                    var PassangerName = $("#txtFirstNameAdults" + i + "").val().trim() + $("#txtLastNameAdults" + i + "").val().trim();
                    var CardHolderName = $("#txtPICCHName").val().trim().split(" ").join("");


                    if (PassangerName.toLowerCase() != CardHolderName.toLowerCase()) {

                        if ($("#txtPICCHName").val().trim().split(" ").length == 2) {
                            var firstName = $("#txtPICCHName").val().trim().split(" ")[0];
                            var lastName = $("#txtPICCHName").val().trim().split(" ")[1];
                            if ($("#txtFirstNameAdults" + i + "").val().trim().toLowerCase() == firstName && $("#txtLastNameAdults" + i + "").val().trim().toLowerCase() == lastName) {
                                ValidationStatus = true;
                                return ValidationStatus;
                            }
                        }

                        if ($("#txtPICCHName").val().trim().split(" ").length == 3) {
                            var firstName = $("#txtPICCHName").val().trim().split(" ")[0];
                            var lastName = $("#txtPICCHName").val().trim().split(" ")[2];
                            if ($("#txtFirstNameAdults" + i + "").trim().val().toLowerCase() == firstName && $("#txtLastNameAdults" + i + "").val().trim().toLowerCase() == lastName) {
                                ValidationStatus = true;
                                return ValidationStatus;
                            }
                        }
...
function GetPaymentOptions() {
    ...
    var CompanyId = sessionStorage.getItem("Id");

    var servicepath = CommonConfiguration.WebSSOURL + "api/CommonService//GetpaymentOptionsByTypeId";

    $.ajax({
        cache: false,
        async: false,
        url: servicepath,
        data: ({ CompanyTypeId: CompanyTypeID, CompanyId: CompanyId }),
        type: "GET",
        cors: true,
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded'
        ...
        success: function (optionList) {
            ...
            if (optionList.length > 0) {
                var combo = $("#selPICCType");//.attr("id", id).attr("name", name);

                $.each(optionList, function (i, el) {
                    combo.append("<option value='" + el.CardValue + "' >" + el.CardType + "</option>");
                });
...
function LoadAirReservationResultData(ResultData) {
    try {
        if (ResultData.Message == "Success" && ResultData.Status == true) {
            ...
            //payment successful
            SelectedAirLineBokingPayment_Data = JSON.parse(localStorage.getItem("SolutionData_App_Data_Traveler|_|SelectedAirLineBokingPayment"))[0];

            var Reqst_Resource = JSON.parse(localStorage.getItem("SolutionData_App_Data_Traveler|_|AirReservationBookingRequest"));

            Reqst_flightsDetailsList_Resource = Reqst_Resource.flightsDetailsList[0];

            //var Reqst_passengersDetail_Resource = JSON.parse(localStorage.getItem("SolutionData_App_Data_Traveler|_|AirReservationBookingRequest"));
            Reqst_passengersDetail_Resource = Reqst_Resource.passengersDetailsList[0];

            //var Reqst_CCD_Resource = JSON.parse(localStorage.getItem("SolutionData_App_Data_Traveler|_|AirReservationBookingRequest"));
            Reqst_CCD_Resource = Reqst_Resource.creditCardOtherDetails[0];
			...
			*/
            ...
            $.each(Result_Data.flights_details, function (FD_Key, FD_Value) {
                ...
                if (SelectionName == "2") {
                    //Round Trip
                    //confirmation_title = "<div class='time-title'>Departure: " + Result_Data.flights_details[0].DepartureDate + "<span><i class='glyphicon glyphicon-play'></i> Return: " + FD_Value.ArrivalDate + "</span></div><p style='padding:5px;'>" + localStorage.getItem("Origin") + "</br><i class='glyphicon glyphicon-play'></i>" + localStorage.getItem("Destination") + "</p>";
                    confirmation_title = "<div class='clearfix'></div><div class='col-sm-6 no-pad-left'><div class='dep-arr'><i class='fa fa-plane fa-1'></i><p style='padding: 0px 0 0px 0;'>" + localStorage.getItem("Origin") + " <small>" + Result_Data.flights_details[0].DepartureDate + "</small></p></div></div><div class='col-sm-6'><div class='dep-arr'><i class='second-i fa fa-plane fa-1'></i><p style='padding: 0px 0 0px 0;'>" + localStorage.getItem("Destination") + "<small>" + FD_Value.ArrivalDate + "</small></p></div></div>";

                }
                else {
                    confirmation_title = "<div class='clearfix'></div><div class='col-sm-6 no-pad-left'><div class='dep-arr'><i class='fa fa-plane fa-1'></i><p style='padding: 0px 0 0px 0;'>" + localStorage.getItem("Origin") + " <small>" + Result_Data.flights_details[0].DepartureDate + "</small></p></div></div>";
                }
                if (SelectionName == "3") {
                    var airPortNameCodeCoList = SolutionDataTraveler("GET", "AirPortNameCodeCoList");
                    var filterOrigin = airPortNameCodeCoList.filter(function (i, n) {
                        return i.airlinecode === Result_Data.flights_details[0].DepartureCityCode;
                    });
                    var origin = filterOrigin[0].airlinename;

                    var filterDestination = airPortNameCodeCoList.filter(function (i, n) {
                        return i.airlinecode === FD_Value.ArrivalCityCode;
                    });
                    var destination = filterDestination[0].airlinename;
                    //confirmation_title = "<div class='time-title'>Departure: " + Result_Data.flights_details[0].DepartureDate + "<span><i class='glyphicon glyphicon-play'></i> Return: " + FD_Value.ArrivalDate + "</span></div><p style='padding:5px;'>" + origin + "</br><i class='glyphicon glyphicon-play'></i>" + destination + "</p>";

                    confirmation_title = "<div class='clearfix'></div><div class='col-sm-6 no-pad-left'><div class='dep-arr'><i class='fa fa-plane fa-1'></i><p style='padding: 0px 0 0px 0;'>" + origin + " <small>" + Result_Data.flights_details[0].DepartureDate + "</small></p></div></div><div class='col-sm-6'><div class='dep-arr'><i class='second-i fa fa-plane fa-1'></i><p style='padding: 0px 0 0px 0;'>" + destination + " <small>" + FD_Value.ArrivalDate + "</small></p></div></div>";
            ...
            $.each(Result_Data.passengers_details, function (PD_Key, PD_Value) {
                ...
                var name = PD_Value.passengername;
                var nameArr = PD_Value.passengername.split(" ").filter(function (e) { return e.replace(/(\r\n|\n|\r)/gm, "") });
                if (nameArr.length == 3) {
                    name = PD_Value.Surname + " " + nameArr[1] + " " + nameArr[2] + " " + nameArr[0];

                }
                if (nameArr.length == 2) {
                    name = PD_Value.Surname + " " + nameArr[1] + " " + nameArr[0];

                }

                Confirmation_PreparedFor += " <p>" + name;// + " " + PD_Value.Surname + "</p>";
            ...
            Confirmation_PreparedFor += " <br /><h5>DATE OF BOOKING : " + $.formatDateTime('DD dd M yy', new Date(new Date().toJSON())) + "</h5></p>";
            Confirmation_PreparedFor += "</div>";
            ...
            $.each(Result_Data.flights_details, function (FD_Key, FD_Value) {
                ...
                if (AirPortNameCodeCoList != undefined) {
                    ...
                    if (ChkDepArray.length == 0) {
                        //if (FD_Value.DepartureCityCode != Departure) {
                        //    ChkDepArray.push(FD_Value.DepartureCityCode);
                        //}
                        //else {
                            ChkDepArray.push(Departure);
                        //}
                ...
                if (AirPortNameCodeCoList != undefined) {
                    ...
                    if (ChkArrivArray.length == 0) {
                        //if (FD_Value.ArrivalCityCode != Arrival) {
                        //    ChkArrivArray.push(FD_Value.ArrivalCityCode);
                        //}
                        //else {
                ...
                if ((ChkDepArray.indexOf(FD_Value.DepartureCityCode) == -1) && (ChkArrivArray.indexOf(FD_Value.DepartureCityCode) == -1)) {
                    ...
                    if (FlitArrivalDateTimeforStopColculation != "") {
                        ...
                        if (SelectionType == "3" && Hour > 24)
                        {
                            Confirmation_FlightDetails += "<table width='100%' border='0' cellspacing='0' cellpadding='0' class='table table-style without_tab layover' style='text-align:center;'>";
                            Confirmation_FlightDetails += "<tr><td colspan='4'></td></tr>";
                            Confirmation_FlightDetails += "</table>";
                        }
                        else {
                            if (SelectionType == "2" && Hour > 24) {
                ...
                if (ChkArrivArray.indexOf(FD_Value.DepartureCityCode) > -1) {
                    ...
                    Confirmation_FlightDetails += "<table width='100%' border='0' cellspacing='0' cellpadding='0' class='table table-style without_tab layover' style='text-align:center;'>";
                    Confirmation_FlightDetails += "<tr><td colspan='4' style='background-color: white;'><i class='fa fa-clock-o' aria-hidden='true'></i> Total Trip Duration:  " + vflighthours + "H " + vflightminutes + "M (" + FlightNumberStop + ")</td></tr>";
                    Confirmation_FlightDetails += "</table>";
                ...
                //FLIGHT DETAILS
                Confirmation_FlightDetails += "<div class='row'><div class='col-md-12'><div class='col-md-12 orng_title'><div class='time-title'><i class='fa fa-plane fa-1'></i>" + FD_Value.DepartureDate +" "+ "<span><span class='arrival-plane'><i class='fa fa-plane fa-1'></i></span> " + FD_Value.ArrivalDate + " </span></div><p><span>Please verify flight times prior to departure</span></p></div></div></div>";

                Confirmation_FlightDetails += "<div class='row'><div class='col-md-12'><div class='col-md-12 no-padding'>";
                Confirmation_FlightDetails += "<table width='100%' border='0' cellspacing='0' cellpadding='0' class='table table-style without_tab' style='text-align:inherit; margin-top:15px;'>";

                if (window.location.href.indexOf("/air") > -1) {
                    // Confirmation_FlightDetails += "<tr><td rowspan='2'><p style='margin-bottom:4px;'><img src='/air/Content/Images/Airlines_Logo/" + FD_Value.MarketingAirline + ".gif'  alt=''> " + "<strong> Flight # " + (FD_Value.AirlineName == null ? FD_Value.MarketingAirline : FD_Value.AirlineName) + "  " + FD_Value.FlightNumber.replace(/^0+/, '') + "</strong>" + "  </p><p>" + (FD_Value.AirlineName == null ? Airline_Name(FD_Value.MarketingAirline) + "</p><p>Operated By " + Airline_Name(FD_Value.MarketingAirline) : Airline_Name(FD_Value.MarketingAirline) + "</p><p>Operated By " + Airline_Name(FD_Value.AirlineName)) + "</p></td>";
                    Confirmation_FlightDetails += "<tr><td><img src='/air/Content/Images/Airlines_Logo/" + FD_Value.MarketingAirline + ".gif'  alt=''></td>  <td><p style='margin-bottom:4px;'> " + "<strong> Flight # " + (FD_Value.AirlineName == null ? FD_Value.MarketingAirline : FD_Value.AirlineName) + "  " + FD_Value.FlightNumber.replace(/^0+/, '') + "</strong>" + "  </p><p>" + (FD_Value.AirlineName == null ? Airline_Name(FD_Value.MarketingAirline) + "</p><p>Operated By " + Airline_Name(FD_Value.MarketingAirline) : Airline_Name(FD_Value.MarketingAirline) + "</p><p>Operated By " + Airline_Name(FD_Value.AirlineName)) + "</p></td>";

                }
                else {
                    //Confirmation_FlightDetails += "<tr><td rowspan='2'><p style='margin-bottom:4px;'><img src='/Content/Images/Airlines_Logo/" + FD_Value.MarketingAirline + ".gif'  alt=''> " + "<strong> Flight # " + (FD_Value.AirlineName == null ? FD_Value.MarketingAirline : FD_Value.AirlineName) + "  " + FD_Value.FlightNumber.replace(/^0+/, '') + "</strong>" + "  </p><p>" + (FD_Value.AirlineName == null ? Airline_Name(FD_Value.MarketingAirline) + "</p><p>Operated By " + Airline_Name(FD_Value.MarketingAirline) : Airline_Name(FD_Value.MarketingAirline) + "</p><p>Operated By " + Airline_Name(FD_Value.AirlineName)) + "</p></td>";
                    Confirmation_FlightDetails += "<tr><td><img src='/Content/Images/Airlines_Logo/" + FD_Value.MarketingAirline + ".gif'  alt=''></td> <td><p style='margin-bottom:4px;'> " + "<strong> Flight # " + (FD_Value.AirlineName == null ? FD_Value.MarketingAirline : FD_Value.AirlineName) + "  " + FD_Value.FlightNumber.replace(/^0+/, '') + "</strong>" + "  </p><p>" + (FD_Value.AirlineName == null ? Airline_Name(FD_Value.MarketingAirline) + "</p><p>Operated By " + Airline_Name(FD_Value.MarketingAirline) : Airline_Name(FD_Value.MarketingAirline) + "</p><p>Operated By " + Airline_Name(FD_Value.AirlineName)) + "</p></td>";
                ...
                Confirmation_FlightDetails += "<td> " + AirPort_Name(FD_Value.DepartureCityCode) + "</p><p>Departing: " + FD_Value.DepartureDate +
                    ...
                    "</p><p>Duration: " + //"<i class='fa fa-plane fa-1'></i>" +
                ...
                if ((SelectionType == "1") || (SelectionType == "3")) {
                    ...
                    if (ChkDepArray.indexOf(FD_Value.ArrivalCityCode) > -1) {
                        ...
                        var vflighthours = Math.floor(Math.abs(consumed_minutes) / 60);
                        var vflightminutes = Math.abs(consumed_minutes) % 60;
                        Confirmation_FlightDetails += "<table width='100%' border='0' cellspacing='0' cellpadding='0' class='table table-style without_tab' style='text-align:center; '>";
                        Confirmation_FlightDetails += "<tr><td colspan='4' style='background-color: white;'><i class='fa fa-clock-o' aria-hidden='true'></i> Total Trip Duration:  " + vflighthours + "H " + vflightminutes + "M (" + FlightNumberStop + ")</td></tr>";
                        Confirmation_FlightDetails += "</table>";
                        sequence = FD_Value.Sequence;
                        consumed_minutes = 0;
            ...
            Confirmation_FlightDetails += "</div>";

            $("#Confirmation_FlightDetails").empty().append(Confirmation_FlightDetails);

            var passengetDetails = "";
            passengetDetails += "<div id='tabs-1'>  ";

            passengetDetails += "<table width='100%' border='0' cellspacing='0' cellpadding='0' class='table table-striped passenger-detail' style='text-align:inherit;'>";
            passengetDetails += "<thead class='thead-inverse'>";
            passengetDetails += "<tr><th>Passenger Name</th><th>Seats</th><th>Class</th><th>Status</th><th>Meals</th></tr></thead>";
            $.each(Result_Data.passengers_details, function (FD_Key, FD_Value) {
                // alert(FD_Value.passengername);


                var name = FD_Value.passengername;
                var nameArr = FD_Value.passengername.split(" ").filter(function (e) { return e.replace(/(\r\n|\n|\r)/gm, "") });
                if (nameArr.length == 3) {
                    name = FD_Value.Surname + " " + nameArr[1] + " " + nameArr[2] + " " + nameArr[0];
                }
                if (nameArr.length == 2) {
                    name = FD_Value.Surname + " " + nameArr[1] + " " + nameArr[0];
                }


                passengetDetails += " <tr><td>" + name + "</td><td>Check-In Required</td><td>Economy</td><td>Confirmed</td><td>Meals</td></tr>";
            ...
            passengetDetails += "</table></div>";

            $("#Confirmation_passengetDetails").empty().append(passengetDetails);
            ...
            Confirmation_PaymentDetail += "<div class=''>";
            Confirmation_PaymentDetail += "<div class='orng_bg top'><div class='col-md-12 col-xs-12'><strong>Payment Detail:</strong></div>";
            
            if (Reqst_CCD_Resource.NameOnCard != undefined && Reqst_CCD_Resource.NameOnCard !="" && Reqst_CCD_Resource.CardType != "CQ") {
                Confirmation_PaymentDetail += "<div class='col-md-4 col-xs-6'>Name On The Card:</div>";
                Confirmation_PaymentDetail += "<div class='col-md-6 no-padding col-xs-6'>" + toTitleCase(Reqst_CCD_Resource.NameOnCard.toLowerCase()); + "</div>";
            }
            Confirmation_PaymentDetail += "</div>";
            Confirmation_PaymentDetail += "</div>";
            Confirmation_PaymentDetail += "<div class=''>";
            Confirmation_PaymentDetail += "<div class=''>";
            if (Reqst_CCD_Resource.CardType != "CQ") {
                Confirmation_PaymentDetail += "<div class='col-md-4 col-xs-6'>Credit Card Charged:</div>";
                Confirmation_PaymentDetail += "<div class='col-md-6 no-padding col-xs-6'>****-****-****-" + Reqst_CCD_Resource.CardNumber.substr(12, 4); + "</div>";
            }
            Confirmation_PaymentDetail += "</div>";
            Confirmation_PaymentDetail += "</div>";
            // Confirmation_PaymentDetail += "<div class='row'>";
            ...
            Total = (parseFloat(Result_Data.currCode_details.PaymentAmount).toFixed(2));

            Confirmation_PaymentDetail += "<div class=''>";
            Confirmation_PaymentDetail += "<div class='col-md-4 col-xs-6'>Amount Charged:</div>";
            Confirmation_PaymentDetail += "<div class='col-md-6 no-padding col-xs-6'> " + sessionStorage.getItem('CurrencyCode') + " " + Total + "</div>";
            Confirmation_PaymentDetail += "</div>";
            ...
            Confirmation_PaymentDetail += "<div class='col-md-12'>";
            Confirmation_PaymentDetail += "<div class='orng_bg'><strong class='billing'>Billing Detail:</strong></div>";
            //Reqst_passengersDetail_Resource.Surname 


            var name = Reqst_passengersDetail_Resource.passengername;
            var nameArr = Reqst_passengersDetail_Resource.passengername.split(" ").filter(function (e) { return e.replace(/(\r\n|\n|\r)/gm, "") });
            if (nameArr.length == 4) {
                name = nameArr[3] + " " + nameArr[1] + " " + nameArr[2] + " " + nameArr[0];

            }
            if (nameArr.length == 3) {
                name = nameArr[2] + " " + nameArr[1] + " " + nameArr[0];
            }

            Confirmation_PaymentDetail += "<div class=''>" + name + "<br>" + Reqst_passengersDetail_Resource.passengerAddress + "<br>" + Reqst_passengersDetail_Resource.email + "<br>" + Reqst_passengersDetail_Resource.PhoneNumber + "</div>";
            ...
            if (n == true) {
                ...
                if (CCEnableAir == "False" || CCEnableAir == "") {
                    error_title_title += "<div class='row'>";
                    error_title_title += "<div class='' style='color:red'>Thank you for booking with us, we are processing your booking.</div>";
...
function LoadAirReservationMulticityResultData(ResultData) {
			...
			*/
            ...
            $.each(Result_Data.flights_details, function (FD_Key, FD_Value) {
                ...
                if (SelectionName == "2") {
                    ...
                    confirmation_title = "<p style='padding:5px;'>Departure: " + Result_Data.flights_details[0].DepartureDate + "</p>" + "<p style='padding:5px;'>" + localStorage.getItem("Origin") + "</br><i class='glyphicon glyphicon-play'></i>" + localStorage.getItem("Destination") + "</p>";
                }
            ...
            $.each(Result_Data.passengers_details, function (PD_Key, PD_Value) {
                ...
                var nameArr = PD_Value.passengername.split(" ").filter(function (e) { return e.replace(/(\r\n|\n|\r)/gm, "") });
                if (nameArr.length == 3) {
                    name = nameArr[2] + " " + nameArr[0] + " " + nameArr[1] + " " + PD_Value.Surname;

                }
                if (nameArr.length == 2) {
                    name = nameArr[0] + " " + PD_Value.Surname + " " + nameArr[1];
            ...
            //Confirmation_PreparedFor += " <p style='padding-top:15px;'>SKYFLIGHT TRAVEL CENTRE<br />" + Result_Data.passengers_details[0].passengerAddress + "<br>" + Reqst_passengersDetail_Resource.PhoneNumber + "<br />" + Reqst_passengersDetail_Resource.PassengerEmail + "";
            Confirmation_PreparedFor += " <p style='padding-top:15px;'>" + ResultData.AgencyDetails;
            Confirmation_PreparedFor += " <br /><h5>DATE OF BOOKING : " + $.formatDateTime('DD dd M yy g:ii a', new Date(new Date().toJSON())) + "</h5></p>";
            Confirmation_PreparedFor += "</div>";
            ...
            $.each(Result_Data.flights_details, function (FD_Key, FD_Value) {
                ...
                if (ChkArrivArray.indexOf(FD_Value.DepartureCityCode) > -1) {
                    ...
                    vflighthours = Math.floor(Math.abs(consumed_minutes) / 60);
                    vflightminutes = Math.abs(consumed_minutes) % 60;
                    if (vflighthours > 0 || vflightminutes > 0) {

                        Confirmation_FlightDetails += "<table width='100%' border='0' cellspacing='0' cellpadding='0' class='table table-style without_tab layover' style='text-align:center; margin-top:15px;'>";
                    ...
                    //Total Trip Duration
                    if (ChkArrivArray.indexOf(FD_Value.DepartureCityCode) == -1) {
                        FlightNumberStop = (Stop == 0) ? "NonStop" : ((Stop == 1) ? "1 Stop" : "2+ Stop");
                ...
                //
                var vSegmentsElapsedTime = FD_Value.ElapsedTime.split('.');
                vflighthours = vSegmentsElapsedTime[0];
                vflightminutes = vSegmentsElapsedTime[1];
                //Flight Details
                consumed_minutes += parseInt(parseInt((vflighthours * 60)) + parseInt(vflightminutes));
                FlitArrivalDateTimeforStopColculation = FD_Value.ArrivalDateTime1;
                //FLIGHT DETAILS
            ...
            $.each(Result_Data.passengers_details, function (FD_Key, FD_Value) {
                ...
                var nameArr = FD_Value.passengername.split(" ").filter(function (e) { return e.replace(/(\r\n|\n|\r)/gm, "") });
                if (nameArr.length == 3) {
                    name = nameArr[2] + " " + nameArr[0] + " " + nameArr[1] + " " + FD_Value.Surname;
                }
                if (nameArr.length == 2) {
                    name = nameArr[0] + " " + FD_Value.Surname + " " + nameArr[1];
                }
            ...
            Confirmation_PaymentDetail += "<div class='col-md-12'>";
            Confirmation_PaymentDetail += "<div class='orng_bg'><strong>Billing Detail:</strong></div>";
            //Reqst_passengersDetail_Resource.Surname 
            //
            Confirmation_PaymentDetail += "<div class=''>" + Reqst_passengersDetail_Resource.passengername + "<br>" + Reqst_passengersDetail_Resource.passengerAddress + "<br>" + Reqst_passengersDetail_Resource.email + "<br>" + Reqst_passengersDetail_Resource.PhoneNumber + "</div>";
...
function AirBookingPaymentForm() {
    try {
        ...
        var FilterAllFlightSegmentArray = JSON.parse(localStorage.getItem('SolutionData_App_Data_Traveler|_|SelectedAirLineBokingPayment'));
        if (FilterAllFlightSegmentArray != 'undefined' && FilterAllFlightSegmentArray != null) {
            ...
            myPassengerDetailsListArray = JSON.parse(localStorage.getItem("SolutionData_App_Data_Traveler|_|GetPassengerDetailsList"));
            $.each(myPassengerDetailsListArray, function (dt_key, dt_value) {
                for (var i = 0; i < parseInt(dt_value.Adults) ; i++) {
                    ...
                    //var givenname = $("#SelTDTitleAdults" + i + "").val().replace(".", "").toUpperCase() + " " +
                    //    $("#txtFirstNameAdults" + i + "").val() + " " +
                    //    ($("#txtMiddleNameAdults" + i + "").val() == "" ? "" : $("#txtMiddleNameAdults" + i + "").val() + " ") +
                    //    ($("#txtLastNameAdults" + i + "").val() == "" ? "" : $("#txtLastNameAdults" + i + "").val() + " ");

                    //var givenname = ($("#txtLastNameAdults" + i + "").val() == "" ? "" : $("#txtLastNameAdults" + i + "").val() + " ") + "/" +  $("#txtFirstNameAdults" + i + "").val() + " " + ($("#txtMiddleNameAdults" + i + "").val() == "" ? "" : $("#txtMiddleNameAdults" + i + "").val() + " ") +" " + $("#SelTDTitleAdults" + i + "").val().replace(".", "").toUpperCase();

                    //var PassengerName = $("#txtLastNameAdults" + i + "").val() + "/" +
                    //    $("#txtFirstNameAdults" + i + "").val() + " " + $("#txtMiddleNameAdults" + i + "").val() + " " +
                    //    $("#SelTDTitleAdults" + i + "").val().replace(".", "").toUpperCase();


                    var PassengerName = $("#SelTDTitleAdults" + i + "").val().replace(".", "").toUpperCase() + " " +
                        $("#txtFirstNameAdults" + i + "").val() + " " + $("#txtMiddleNameAdults" + i + "").val() + " " +
                         $("#txtLastNameAdults" + i + "").val();
                    ...
                    passengers_Details_List.push({
                        ...
                        passengerAddress: ($("#txtBCICity").val() == "" ? $('#selBCICountry').val() : $("#txtBCICity").val() + " , " + $('#selBCICountry').val()),
                ...
                for (var i = 0; i < parseInt(dt_value.Childrens) ; i++) {
                    ...
                    //var givenname = $("#SelTDTitleChildrens" + i + "").val().replace(".", "").toUpperCase() + " " +
                    //    $("#txtFirstNameChildrens" + i + "").val() + " " +
                    //    ($("#txtMiddleNameChildrens" + i + "").val() == "" ? "" : $("#txtMiddleNameChildrens" + i + "").val() + " ") +
                    //    ($("#txtLastNameChildrens" + i + "").val() == "" ? "" : $("#txtLastNameChildrens" + i + "").val() + " ");

                    var PassengerName = $("#SelTDTitleChildrens" + i + "").val().replace(".", "").toUpperCase() + " " + $("#txtFirstNameChildrens" + i + "").val() + " " + $("#txtMiddleNameChildrens" + i + "").val() + " " + $("#txtLastNameChildrens" + i + "").val();

                    //var givenname = ($("#txtLastNameChildrens" + i + "").val() == "" ? "" : $("#txtLastNameChildrens" + i + "").val() + " ") + "/"
                    //    + $("#txtFirstNameChildrens" + i + "").val() + " " + ($("#txtMiddleNameChildrens" + i + "").val() == "" ? "" : $("#txtMiddleNameChildrens" + i + "").val() + " ") + " " + $("#SelTDTitleChildrens" + i + "").val().replace(".", "").toUpperCase();


                    //var PassengerName = $("#txtLastNameChildrens" + i + "").val() + "/" + $("#txtFirstNameChildrens" + i + "").val() + " "
                    //            + $("#txtMiddleNameChildrens" + i + "").val() + " " +
                    //            $("#SelTDTitleChildrens" + i + "").val().replace(".", "").toUpperCase();
                ...
                for (var i = 0; i < parseInt(dt_value.Infants) ; i++) {
                    ...
                    //var givenname = $("#SelTDTitleInfants" + i + "").val().replace(".", "").toUpperCase() + " " + $("#txtFirstNameInfants" + i + "").val() + " " +
                    //    ($("#txtMiddleNameInfants" + i + "").val() == "" ? "" : $("#txtMiddleNameInfants" + i + "").val() + " ") +
                    //    ($("#txtLastNameInfants" + i + "").val() == "" ? "" : $("#txtLastNameInfants" + i + "").val() + " ");


                    //var givenname = ($("#txtLastNameInfants" + i + "").val() == "" ? "" : $("#txtLastNameInfants" + i + "").val() + " ") + "/" +
                    //    $("#txtFirstNameInfants" + i + "").val() + " " +
                    //    ($("#txtMiddleNameInfants" + i + "").val() == "" ? "" : $("#txtMiddleNameInfants" + i + "").val() + " ") + " " +
                    //    $("#SelTDTitleInfants" + i + "").val().replace(".", "").toUpperCase();

                    //var PassengerName = $("#txtLastNameInfants" + i + "").val() + "/"+$("#txtFirstNameInfants" + i + "").val()+ " " +
                    //    $("#txtMiddleNameInfants" + i + "").val() + " " +
                    //    $("#SelTDTitleInfants" + i + "").val().replace(".", "").toUpperCase();
            ...
            try {
                ...
                try { CardType = $("#selPICCType").val().trim(); } catch (e) { CardType = ""; }
                try { CardNumber = $("#txtPICCNumber").val().trim(); } catch (e) { CardNumber = "";}
                try { CVVNumber = $("#txtPIVCNumber").val().trim(); } catch (e) { CVVNumber = ""; }
                try { Address = $("#txtBCICity").val().trim() + "  " + $("#selBCICountry").val().trim() + "  " + $("#txtBCIZip").val().trim(); } catch (e) { Address = ""; }
                try { CardExpiryDate = $("#selectPICCYear").val().trim() + "-" + $("#selectPICCMonth").val().trim(); } catch (e) { CardExpiryDat = ""; }
                try { NameOnCard = $("#txtPICCHName").val().trim(); } catch (e) { NameOnCard = ""; }
                try { AirLineCode1 = FilterAllFlightSegment_Array[0].MarketingAirline.Code; } catch (e) { AirLineCode1 = ""; }
                try { CurrCode = AirItineraryPricingInformation.CurrencyCode; } catch (e) { CurrCode = ""; }
                try { PaymentAmount = parseFloat(AirItineraryPricingInformation.Amount).toFixed(2); } catch (e) { PaymentAmount = ""; }
                try { CCFee = $("#lblCCFee").text(); } catch (e) { CCFee = ""; }
            ...
            var SaveRecords = {
                ...
                Locator: "",
                UserId: localStorage.getItem("loginUID") == null ? "" : localStorage.getItem("loginUID"),
                ...
                selectionkey: parseInt(sessionStorage["SelectionType"])
            };
            ...
            var PolicyDetails = {
                ...
                Total_Insurance_Price: $('#hdTotalPolicyAmount').val()
            }
            // 
            var APP_REQ = {
            ...
            function ResultCallBackSuccess(e, xhr, opts) {
                ...
                //if (SelectionType === "3") {
                   // LoadAirReservationMulticityResultData(ResultData);
                //}
               // else {
            ...
            // 
            localStorage.setItem("SolutionData_App_Data_Traveler|_|AirReservationBookingRequest", JSON.stringify(APP_REQ))
...
$(document).ready(function () {
    ...
    $("#radiobookinsurance").on('click', function () {
        ...
        if (ValidateInsuranceData()) {
            ...
            //var Pass_Cnt = $('#type0').closest('td').next().text();

            for (var i = 0; i < parseInt(Pass_Cnt) ; i++) {

                var type = $('#type' + i).text().trim();
                // var type = $('#type0').text().trim();
                var typeid = "";
                if (type == "Adult") {
                    typeid = "Adults";
                }
            ...
            function ResultCallBackSuccess(e, xhr, opts) {
                ...
                var Total_Insurance_Price = parseFloat(App_Data.RESPONSE.Products[0].PRICE) + parseFloat(App_Data.RESPONSE.Products[0].TAX)

                $('#hdTotalPolicyAmount').val(parseFloat(Total_Insurance_Price));
                var html = "";
                ...
                for (var j = 0; j < App_Data.RESPONSE.Products[0].PAXES.length; j++) {
                    if (j == 0) {
                        html += "<p>Passenger " + parseInt(j + 1) + " Insurance Price " + ": " + sessionStorage.getItem('CurrencyCode') + " " + App_Data.RESPONSE.Products[0].PAXES[j].PRICE + "</p>";
                        $('#passenger' + j).val(parseFloat(App_Data.RESPONSE.Products[0].PAXES[j].PRICE) + parseFloat(App_Data.RESPONSE.Products[0].PAXES[j].Tax));
                    }
                    else if (j == 1) {
                        html += "<p>Passenger " + parseInt(j + 1) + " Insurance Price " + ": " + sessionStorage.getItem('CurrencyCode') + " " + App_Data.RESPONSE.Products[0].PAXES[j].PRICE + "</p>";
                        $('#passenger' + j).val(parseFloat(App_Data.RESPONSE.Products[0].PAXES[j].PRICE) + parseFloat(App_Data.RESPONSE.Products[0].PAXES[j].Tax));
                        ...
                        html += "<p>Passenger " + parseInt(j + 1) + " Insurance Plan " + ": " + sessionStorage.getItem('CurrencyCode') + " " + App_Data.RESPONSE.Products[0].PAXES[j].PRICE + "</p>";
                        $('#passenger' + j).val(parseFloat(App_Data.RESPONSE.Products[0].PAXES[j].PRICE) + parseFloat(App_Data.RESPONSE.Products[0].PAXES[j].Tax));
                    }
...
function OnLoadCreateVailidatePassengerDetailsDynamically() {
    try {
        ...
        $.each(myPassengerDetailsListArray, function (dt_key, dt_value) {
            ...
            if (parseInt(dt_value.Adults) > 0) {
                AppPassengerDetailsData += "<div class='col-md-12'><div class='form-group col-md-1 ' style='width:100%; height:27px;  margin-left: 0px;'>Adults</div></div>";
            }
            AppPassengerDetailsData += "<input type='hidden' id='passenger" + count + "' />";
            count += 1;
            for (var i = 0; i < parseInt(dt_value.Adults) ; i++) {
                AppPassengerDetailsData += "<div class='col-md-12 table-responsive'>";
                AppPassengerDetailsData += "<div class='form-group col-md-1'><label class='form-control adult-1' style='width:auto; border: none; height:27px; margin-top: 0px; margin-left: 10px;'>" + (i + 1) + ":</label></div>";
                ...
                AppPassengerDetailsData += "<div class='form-group col-md-2'><input type='text' autocomplete='off' id='txtFirstNameAdults" + i + "' name='txtFirstNameAdults" + i + "' placeholder='First Name*'  class=' form-control' style='width: 100%;float:left; height: 27px; margin-top: 0px;'  /></div>";
                ...
                AppPassengerDetailsData += "<div class='form-group col-md-2'><input type='text' autocomplete='off'  id='txtLastNameAdults" + i + "' name='txtLastNameAdults" + i + "' placeholder='Last Name*'  class='  form-control' style='width: 100%;float:left; height: 27px; margin-top: 0px;'  /></div>";
            ...
            if (parseInt(dt_value.Childrens) > 0) {
                AppPassengerDetailsData += "<div class='col-md-12'><div class='form-group col-md-1' style='width:100%; height:27px;  margin-left: 0px;'>Children</div></div>";
            }
            for (var i = 0; i < parseInt(dt_value.Childrens) ; i++) {
                AppPassengerDetailsData += "<div class='col-md-12 table-responsive'>";
                AppPassengerDetailsData += "<div class='form-group col-md-1'><label class='form-control' style='width:auto; height:27px; border:none; margin-top: 0px; margin-left: 10px;'>" + (i + 1) + ":</label></div>";
                ...
                AppPassengerDetailsData += "<div class='form-group col-md-2'><input type='text' autocomplete='off'  id='txtFirstNameChildrens" + i + "' name='txtFirstNameChildrens" + i + "' placeholder='First Name*'  class='col-xs-10 col-sm-5' style='width: 100%;float:left; height:27px; margin-top: 0px; '  /></div>";
                ...
                AppPassengerDetailsData += "<div class='form-group col-md-2'><input type='text' autocomplete='off'  id='txtLastNameChildrens" + i + "' name='txtLastNameChildrens" + i + "' placeholder='Last Name*'  class='col-xs-10 col-sm-5' style='width: 100%;float:left; height:27px; margin-top: 0px; '  /></div>";
            ...
            if (parseInt(dt_value.Infants) > 0) {
                AppPassengerDetailsData += "<div class='col-md-12'><div class='form-group col-md-1' style='width:100%; height:27px;  margin-left: 0px;'>Infants</div></div>";
            }
            for (var i = 0; i < parseInt(dt_value.Infants) ; i++) {
                AppPassengerDetailsData += "<div class='col-md-12 table-responsive'>";
                AppPassengerDetailsData += "<div class='form-group col-md-1'><label class='form-control' style='width:100%; height:27px; margin-top: 0px; border:none;  margin-top: 1px; margin-left: 10px;'>" + (i + 1) + ":</label></div>";
                ...
                AppPassengerDetailsData += "<div class='form-group col-md-2'><input type='text' autocomplete='off'  id='txtFirstNameInfants" + i + "' name='txtFirstNameInfants" + i + "' placeholder='First Name*'  class='col-xs-10 col-sm-5' style='width: 100%; float:left; height:27px; margin-top: 0px; '  /></div>";
                ...
                AppPassengerDetailsData += "<div class='form-group col-md-2'><input type='text' autocomplete='off'  id='txtLastNameInfants" + i + "' name='txtLastNameInfants" + i + "' placeholder='Last Name*'  class='col-xs-10 col-sm-5' style='width: 100%;float:left; height:27px; margin-top: 0px; '  /></div>";
            ...
            if (parseInt(dt_value.OnSeat) > 0) {
                AppPassengerDetailsData += "<div class='col-md-12'><div class='form-group col-md-1' style='width:100%; height:27px;  margin-left: 0px;'>OnSeat</div></div>";
            }
            for (var i = 0; i < parseInt(dt_value.OnSeat) ; i++) {
                AppPassengerDetailsData += "<div class='col-md-12 table-responsive'>";
        ...
        ChildrenDOBStartDate = ChildrenDOBStartDate.getFullYear() - 10;
...
function BindDisplayResutModuleDataNew() {
    ...
    try {
        ...
        var SelectedAirLineAllFlightSegment_Data = JSON.parse(localStorage.getItem('SolutionData_App_Data_Traveler|_|SelectedAirLineBokingPayment'));
        if (SelectedAirLineAllFlightSegment_Data != 'undefined' && SelectedAirLineAllFlightSegment_Data != null) {
            var dynamicDetails = "";
            sessionStorage["departarraynew"] = "[\"" + SelectedAirLineAllFlightSegment_Data[0].FlightSegments[0].DepartureAirport.LocationCode + "\"]";
            sessionStorage["arrivalarraynew"] = "[\"" + SelectedAirLineAllFlightSegment_Data[0].FlightSegments[0].ArrivalAirport.LocationCode + "\"]";

            localStorage["OriginNew"] = "[\"" + SelectedAirLineAllFlightSegment_Data[0].FlightSegments[0].DepartureAirport.LocationCode + "\"]";
            localStorage["DestinationNew"] = "[\"" + SelectedAirLineAllFlightSegment_Data[0].FlightSegments[0].ArrivalAirport.LocationCode + "\"]";
            ...
            if (SelectedAirLineAllFlightSegment_Data.length > 0) {
                ...
                $.each(FightsArray, function (All_key, All_value) {
                    ...
                    LastDepDate = All_value.OriginDestinationFlight[OriginDestLength - 1].FlightSegments[FlightSegments - 1].DepartureDateTime;
                    ...
                    $.each(All_value.OriginDestinationFlight, function (FlightSegment_key, FlightSegment_value) {
                        ...
                        localStorage["flighthours" + FlightSegment_key] = Math.trunc(SegmentsElapsedTime / 60);
                        localStorage["flightminutes" + FlightSegment_key]  = SegmentsElapsedTime % 60;
                ...
                $.each(SelectedAirLineAllFlightSegment_Data, function (All_key, All_value) {
                    ...
                    dynamicDetails += "<div class='price-detail-table' style=''>";
                    dynamicDetails += "<table class='table without_tab'><thead id='pricedtl' style='border-top:1px;'><tr>";
                    dynamicDetails += "<th>Passanger Type</th>";
                    dynamicDetails += "<th style=''>Number of Passanger</th>";
                        ...
                        if (AirItinerary == 0) {
                            ...
                            for (var i = 0; i < AirItineraryPricingInformation.PTC_FareBreakdowns.PTC_FareBreakdown.length; i++) {
                                ...
                                if (AirItineraryPricingInformation.PTC_FareBreakdowns.PTC_FareBreakdown[i].PassengerTypeQuantity.Code == 'ADT')
                                    dynamicDetails += "<tr><td data-label='Passanger Type' id='type" + i + "'> Adult</td>";
                                else if (AirItineraryPricingInformation.PTC_FareBreakdowns.PTC_FareBreakdown[i].PassengerTypeQuantity.Code == 'C07')
                                    dynamicDetails += "<tr><td data-label='Passanger Type' id='type" + i + "'> Child</td>";
                                else if (AirItineraryPricingInformation.PTC_FareBreakdowns.PTC_FareBreakdown[i].PassengerTypeQuantity.Code == 'INF')
                                    dynamicDetails += "<tr><td data-label='Passanger Type' id='type" + i + "'> Infant</td>";

                                dynamicDetails += "<td  data-label='Number of Passanger'>" + AirItineraryPricingInformation.PTC_FareBreakdowns.PTC_FareBreakdown[i].PassengerTypeQuantity.Quantity + "</td>";
                                dynamicDetails += "<td  data-label='Base Fare' id='tripcost" + i + "' class='text-right'>" + parseFloat(AirItineraryPricingInformation.PTC_FareBreakdowns.PTC_FareBreakdown[i].PassengerTypeQuantity.Quantity * AirItineraryPricingInformation.PTC_FareBreakdowns.PTC_FareBreakdown[i].PassengerFare.EquivFare.Amount).toFixed(2) + "</td>";
                                dynamicDetails += "<td  data-label='Taxes &amp; Fees' id='tax" + i + "' class='text-right'>" + parseFloat(AirItineraryPricingInformation.PTC_FareBreakdowns.PTC_FareBreakdown[i].PassengerTypeQuantity.Quantity * AirItineraryPricingInformation.PTC_FareBreakdowns.PTC_FareBreakdown[i].PassengerFare.Taxes.TotalTax.Amount).toFixed(2) + "</td>";
                                dynamicDetails += "<td data-label='Total Price' id='price" + i + "' class='text-right'>" + parseFloat(AirItineraryPricingInformation.PTC_FareBreakdowns.PTC_FareBreakdown[i].PassengerTypeQuantity.Quantity * AirItineraryPricingInformation.PTC_FareBreakdowns.PTC_FareBreakdown[i].PassengerFare.TotalFare.Amount).toFixed(2) + "</td></tr>";
                            ...
                            dynamicDetails += "<tr class='no-border-line'> <td class='last-td' data-label=''></td><td class='last-td' data-label=''></td><td class='last-td' data-label=''></td><td><span class='grand'>Grand Total</span></td><td data-label='Grand Total'>" + total.toFixed(2) + " " + currencyCode + "</td></tr><tr><td colspan='5' class='note-txt' data-label=''>Please note: All fares are quoted in " + sessionStorage.getItem('CurrencyCode') + ". Some airlines may charge baggage fees.</td></tr>";

                            //dynamicDetails += "<tr><td colspan='3'></td><td>Grand Total</td><td class='text-right'>" + total.toFixed(2) + " " + currencyCode + "</td></tr>";
                            //dynamicDetails += "<tr><td colspan='5' class='note-txt' >Please note: All fares are quoted in  " + sessionStorage.getItem('CurrencyCode') + ". Some airlines may charge baggage fees.</td></tr>";
							dynamicDetails += "</tbody>";
                    dynamicDetails += "</table>";
                            //dynamicDetails += "<div class='quoted-note'>Please note: All fares are quoted in " + sessionStorage.getItem('CurrencyCode') + ". Some airlines may charge baggage fees.</div>";
