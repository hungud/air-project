...
function GetBlockedAirlines() {
    var domainName = localStorage.getItem("domain");
    var servicepath = CommonConfiguration.searchBoxService + "/GetBlockedAirlines";
        ...
        if (FilterAllFlightSegment_Array.length > 0) {
            ...
            $.each(FilterAllFlightSegment_Array, function (FlightSegment_key, FlightSegment_value) {
                ...
                if (FlightSegment_value.FlightSegments.length > 0) {
                    ...
                    var NetFare = 0;
                    //debugger;
                    $.each(Markup_Array, function (Markup_key, Markup_value) {
                    ...
                    if (PassengerTypeQuantity == "ADT") {
                        //Public Fare
                        if (MarkupType == "value") {
                            FareMarkup = PublishedFare;
                        }
                        else if (MarkupType == "percentage") {
                            FareMarkup = parseFloat(flightSegment.AirItineraryPricingInfo.ItinTotalFare.TotalFare.Amount) * PublishedFare / 100;
                        }
                    }
                    else {
                        FareMarkup = NetFare;
            ...
            if ((IsDirectFlight == true) && (DirectFlightCount == 0)) {
...
function QueryStringDataSearchCondition(SearchType, SearchContentData) {
    ...
    $('#ddlAirline').val(SearchContentData.Airline.toUpperCase());
        ...
        if (GetQueryStringParameterValues('Search') != null) {
            ...
            function ResultCallBackError(e, xhr, opts) {
                ...
                if (e.responseText == "Authentication Required") {
                    ...
                    //alert(sessionStorage.getItem("IsFromPaymentPage") );
                    var urlpart = document.location.protocol + "//" + document.domain + (location.port == "" ? "" : ":" + location.port) + "/Account/Login?returnUrl=/";
                    var urlpart2 = $.urlParam('Search');
                    var url = urlpart + urlpart2;
                    window.location.href = document.location.protocol + "//" + document.domain + (location.port == "" ? "" : ":" + location.port) + "/Account/Login?returnUrl=/" + $.urlParam('Search');
                    return false;
                }
                else {
                    ShowWaitProgress();
            ...
            var Reqst_Resource = {
                ...
                IsAuthenticated: $('#is_loggedin').val(),
...
function OnLoadGetDataList() {
    ...
    try {
        var RequestContentData = '';
        var airLineCode = $("#ddlAirline").val();
